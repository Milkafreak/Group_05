...
class Energy:
    ...
    def plot_consumption(self,country: str,normalize: bool):
        ...
        if country not in self.data["country"].unique():
            ...
            values = self.data[self.data["country"] == country][self.data.columns[self.data.columns.str.contains("_consumption")]] #Take values of all _consumption columns
            values = values.fillna(0) #Fill empty values with 0 
            x_scaled = values.div(values.sum(axis=1), axis=0).reset_index(drop=True) #normalize values
            df2 = pd.DataFrame(x_scaled,columns=values.columns) #make new DF with scaled values
